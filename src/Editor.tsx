  ...
  const fileSelected = () => {
    ...
    if (files === null || files.length === 0) {
      ...
      reader.onload = () => {
        ...
        sizeImg.onload = () => {
          const { width: cardFrameWidth = 0, height: cardFrameHeight = 0 } =
            cardFrameElement.current?.getBoundingClientRect() || {};

          const widthRatio =
            sizeImg.naturalWidth <= cardFrameWidth * 0.9
              ? 1
              : (cardFrameWidth * 0.9) / sizeImg.naturalWidth;
          const heightRatio =
            sizeImg.naturalHeight <= cardFrameHeight * 0.9
              ? 1
              : (cardFrameHeight * 0.9) / sizeImg.naturalHeight;
    ...
    <>
      <div>
        <FormControl component="fieldset">
          ...
          >
            {imageSizeButtons.map((data) => (
              <FormControlLabel
                key={`imagesize_${data.value}`}
                value={data.value}
                control={<Radio />}
                label={data.label}
                id={`imagesize_${data.value}`}
              />
            ))}
          </RadioGroup>
          <Grid container component="label" alignItems="center">
            ...
            <Grid item>
              <Switch
                checked={dragActive}
                onChange={(e) => setDragActive(e.target.checked)}
                disabled={imageSize !== "free"}
              />
            </Grid>
            <Grid item>動かす</Grid>
          </Grid>
      ...
      <div>
        <FormControl component="fieldset">
          ...
          >
            {rarityButtons.map((data) => (
              <FormControlLabel
                key={`rarity_${data.value}`}
                value={data.value}
                control={<Radio />}
                label={data.label}
                id={`rarity_${data.value}`}
              />
            ))}
          </RadioGroup>
        </FormControl>
        ...
        <FormControl component="fieldset">
          ...
          >
            {attributeButtons.map((data) => (
              <FormControlLabel
                key={`attribute_${data.value}`}
                value={data.value}
                control={<Radio />}
                label={data.label}
                id={`attribute_${data.value}`}
              />
            ))}
          </RadioGroup>
        </FormControl>
      ...
      <div>
        ...
        <InputArea>
          <TextField
            label="能力の説明"
            value={abilityNote}
            onChange={createSet(setAbilityNote)}
            fullWidth
          />
        </InputArea>
        <InputArea>
          <TextField
            ...
            label="説明"
            value={description}
            onChange={createSet(setDescription)}
            fullWidth
          />
        </InputArea>
        <InputArea>
          <TextField
            multiline
            label="QRコード"
            helperText="QRコードにしたいテキスト(文字列)やURLがあれば入力してね"
            value={qrText}
            onChange={createSet(setQrText)}
            fullWidth
          />
        </InputArea>
      </div>
